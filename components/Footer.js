export default () => (
    <footer className="footer">
        <div className="container">
            <div className="columns">
                
                <div className="column">
                    <div className="logo">
                        <img src="static/img/logo-1024x1024.png" />
                        <h2 className="title is-4"><b>Squoosh</b><br/>
                        Desktop app</h2>
                    </div>
                </div>

                <div className="column">
                    <h3 className="title is-4">Credits</h3>
                    <p>Squoosh Desktop App is an <a href="https://electronjs.org/" target="_blank">Electron</a> version of Squoosh app.</p>
                    <p>Squoosh is an open source product made by <a href="https://github.com/GoogleChromeLabs" target="_blank">Google Chrome Labs</a>.</p>
                    <p>The desktop app is a project of <a href="https://mebenedetto.com" target="_blank">Matias Benedetto</a></p>
                </div>

                <div className="column">
                    <h3 className="title is-4">Contribute</h3>
                    <p>Squoosh Desktop App is an open source project, you can contribute in <a href="https://github.com/matiasbenedetto/squoosh-desktop-app" target="_blank">GitHub</a>.</p>
                </div>

                <div className="column">
                    <h3 className="title is-4">Links</h3>
                    <p><a href="https://github.com/matiasbenedetto/squoosh-desktop-app" target="_blank">Project's repo</a></p>
                    <p><a href="http://mebenedetto.com" target="_blank">Author's blog</a></p>
                    <p><a href="https://squoosh.app" target="_blank">Squoosh web app</a></p>
                    <p><a href="https://github.com/matiasbenedetto/squoosh-desktop-app-website" target="_blank">Website's repo</a></p>
                </div>
            </div>
        </div>

    </footer>
)
